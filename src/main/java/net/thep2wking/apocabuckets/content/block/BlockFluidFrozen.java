...
public class BlockFluidFrozen extends ModBlockFluidApocalypticBase {
    ...
    public void updateTick(World world, BlockPos pos, IBlockState state, Random rand) {
        ...
        if (!world.isRemote && world instanceof WorldServer) {
            ((WorldServer) world).addScheduledTask(() -> handleUpdate(world, pos, state, rand));
        }
    }

    private void handleUpdate(World world, BlockPos pos, IBlockState state, Random rand) {
        if (shouldBeIce(world, pos, rand)) {
            IBlockState newState = (state.getBlock() instanceof BlockIce || state.getBlock() instanceof BlockPackedIce)
